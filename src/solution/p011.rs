...
pub fn solve(grid: &str, len: usize) -> usize {
    let grid = grid
        .lines()
        .map(|row| {
            row.trim()
    ...
    // rows
    lines.extend((0..height).map(|y| (0..width).map(|x| (x, y)).collect()));
    // cols
    lines.extend((0..width).map(|x| (0..height).map(|y| (x, y)).collect()));
    // top 2 right diagonal
    lines.extend((0..width).map(|i| {
        let (x0, y0) = (i, 0);
        (0..width - x0).map(|j| (x0 + j, y0 + j)).collect()
    }));
    // left 2 bottom diagonal
    lines.extend((0..height - 1).map(|i| {
        let (x0, y0) = (0, i + 1);
        (0..height - y0).map(|j| (x0 + j, y0 + j)).collect()
    }));
    // top 2 left diagonal
    lines.extend((0..width).map(|i| {
        let (x0, y0) = (i, 0);
...
mod test {
    ...
    fn test() {
        ...
        assert_eq!(solve(grid, 4), 70600674);
    }
}
